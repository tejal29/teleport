...
This configuration enables the HTTP input, which will listen on port `9601` and
assign logs to the `teleport-audit` index when the Universal Forwarder sends
them to Splunk. `enableSSL = 1` enables SSL for the input, which promotes
defense in depth if you are running the Universal Forwarder and the Event
Handler on teh same host and is necessary for security if not.
...
(if a field already exists in this config file, replace it):
...
{/* TODO: How to authenticate the Forwarder with Splunk?*/}
