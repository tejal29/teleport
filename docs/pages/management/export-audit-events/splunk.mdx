...
which stores them in Splunk for visualization and alerting. 

## Prerequisites

(!docs/pages/includes/commercial-prereqs-tabs.mdx!)

- Splunk Observability Cloud or Splunk Enterprise v9.0.1 or above. 

{/* TODO: Indicate open ports on Splunk Enterprise?*/}

- A Linux host where you will run the Teleport Event Handler plugin and Splunk
  Universal Forwarder. The host must have port `0000` exposed to traffic from
  the Teleport Auth Service host (or your Teleport Cloud tenant). {/* TODO:
  update port*/} The Universal Forwarder must be installed on the host already.

  While you can run the Event Handler plugin and Universal
  Forwarder on separate hosts, this guide will assume that the two are running
  on the same host.
