...
- A Linux host where you will run the Teleport Event Handler plugin and Splunk
  ...
  already. This guide will assume that you are running the Universal Forwarder
  on the same host as the Event Handler.

  If you run the Event Handler and Universal Forwarder on the same host, there
  is no need for an open intake port. However, if you run the Universal
  Forwarder on a separate host from the Event Handler, you will need to open a
  port on the Universal Forwarder host to traffic from the Event Handler. This
  guide assumes that the Universal Forwarder is listening on port `8888`.
