...
- A Linux host where you will run the Teleport Event Handler plugin.

- On Splunk Enterprise, port `9997` should be open to traffic from the host
  running the Teleport Event Handler.
...
cluster. In this section, you will install the Event Handler plugin on a Linux
host and generate credentials that the plugin will use for authentication.
...
<Tabs dropdownCaption="Method" dropdownSelected="Executable">
  ...
  You must have Docker Desktop installed on your Event Handler host. 
...
will refer to when generating credentials for the plugin:
...
## Step 2/4. Configure Splunk

Configure Splunk to accept logs from the Teleport Event Handler plugin via
Splunk's HTTP Event Collector. The Event Handler sends audit events as HTTP POST
...
Now that you have configured Splunk to receive logs via HTTP, configure the
Event Handler to send audit events to Splunk.
