...
{/* TODO: Determine whether to fill in the commented-out fields*/}
[http]
port = 9061
disabled = false
# outputgroup = <string>
index = teleport-audit
# sourcetype = <string>
enableSSL = 1
```

This configuration enables the HTTP input, which will listen on port `9061` and
forard logs to Splunk, assigning them to the `teleport-audit` index. `enableSSL
= 1` enables SSL for the input, which promotes defense in depth if you are
running the Universal Forwarder and the Event Handler on the same host and is
necessary for security if not.
...
(if this file already exists,  unassign the following fields in the
`[sslConfig]` section):
...
sslRootCAPath =
sslPassword = 
certCreateScript = true
```

Assign `serverCert` to the path to the `server.crt` file you generated earlier
and `sslRootCAPath` to the path to `ca.crt`.
...
The `certCreateScript` value is set to the path of the `true` command, which
returns a successful exit status after doing nothing. This overrides the
default, which generates certificates automatically, so we can use the
certificate generated by the `teleport-event-handler configure` command earlier.

Ensure that the Universal Forwarder can read the TLS credentials you provided by
changing their ownership to the `splunk` user:

```code
$ sudo chown splunk:splunk server.crt ca.crt
```
...
your Universal Forwarder's `http` input earlier, `https://localhost:9061`.
Change `forward.fluentd.session-url` to the same value with the root URL path:
`https://localhost:9061/`. (Replace `localhost` with your Universal Forwarder's
IP address or domain name if you are running the Universal Forwarder and Event
Handler on separate hosts.)
